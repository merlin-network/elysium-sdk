...
On Elysium mainnet, we only utilize one identifier, with a time interval of `one day`.
...
```sh
elysiumCore query epochs epoch-infos
```
...
```sh
elysiumCore query epochs current-epoch [identifier]
```
...
```sh
elysiumCore query epochs current-epoch day
```
